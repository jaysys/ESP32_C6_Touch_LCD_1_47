...
/*
 * ════════════════════════════════════════════════════════════
 *  v9 MIGRATION NOTES  (things moved from conf to code)
 * ════════════════════════════════════════════════════════════
 *
 *  1. LV_COLOR_16_SWAP (removed from v9 conf)
 *     → Call in setup() AFTER lv_display_create():
 *       lv_display_set_color_format(disp, LV_COLOR_FORMAT_RGB565_SWAPPED);
 *     → Only needed if your display has byte-swapped SPI (ST7789 usually does).
 *       Test first without it; if colors look wrong, add it.
 *
 *  2. LV_TICK_CUSTOM / millis() (removed from v9 conf)
 *     → Call in setup() AFTER lv_init():
 *       lv_tick_set_cb((lv_tick_get_cb_t)millis);
 *     → This replaces the v8 LV_TICK_CUSTOM + LV_TICK_CUSTOM_SYS_TIME_EXPR.
 *
 *  3. LV_MEM_CUSTOM (replaced by LV_USE_STDLIB_MALLOC above)
 *     → LV_STDLIB_CLIB = use standard malloc/free/realloc automatically.
 *       No explicit custom include/alloc/free defines needed.
 *
 * ════════════════════════════════════════════════════════════
 */
 ...
 */
#define LV_USE_STDLIB_MALLOC    LV_STDLIB_CLIB    /* Use system malloc — replaces v8 LV_MEM_CUSTOM 1 */
 ...
 * - LV_STDLIB_CUSTOM:      Implement the functions externally
 */
#define LV_USE_STDLIB_STRING    LV_STDLIB_CLIB    /* match malloc — use system memcpy/memset */
 ...
 * - LV_STDLIB_CUSTOM:      Implement the functions externally
 */
#define LV_USE_STDLIB_SPRINTF   LV_STDLIB_CLIB    /* match malloc — use system snprintf */
...
#define LV_DEF_REFR_PERIOD  30      /**< [ms] — kept from v8 */

/** Default Dots Per Inch. Used to initialize default sizes such as widgets sized, style paddings.
 * (Not so important, you can adjust it to modify default sizes and spaces.) */
#define LV_DPI_DEF 130              /**< [px/inch] — kept from v8 */
...
#define LV_FONT_MONTSERRAT_48 1  /* home screen clock */
...
#define LV_USE_GIF 1
    ...
    #define LV_GIF_CACHE_DECODE_DATA 0  /* set to 1 to trade RAM for decode speed — leave 0 for large GIFs */
#endif
...
#define LV_BUILD_EXAMPLES 0  /* disable — saves flash on ESP32 */

/** Build the demos */
#define LV_BUILD_DEMOS 0  /* disable — saves flash on ESP32 */
